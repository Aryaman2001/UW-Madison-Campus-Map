...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path source) {
			...
			// iterating and copying through source
			for (T data : source.dataSequence) {
		...
		public void extend(Edge edge) {
			...
			this.distance = this.distance + edge.weight;
	...
	protected Path dijkstrasShortestPath(T start, T end) {		
		// checking whether the start and end vertex exist or not
		if ((!(vertices.containsKey(end))) || (!(vertices.containsKey(start)))) {
			throw new NoSuchElementException();
		} 

		// to store the discovered paths
		PriorityQueue<Path> discovered = new PriorityQueue<>(); 		
		Path path = new Path(vertices.get(start)); 
		Path tempPath = null;
		
		// to store the path (shortest) to be returned
		Hashtable<T, Path> returnPath = new Hashtable<>(); 		
		returnPath.put(start, path);
		discovered.add(path);
		
		while (!discovered.isEmpty()) {
			path = discovered.remove();
			Iterator<Edge> iterator = path.end.edgesLeaving.iterator();
			
			while (iterator.hasNext()) {
				Edge edge = iterator.next();
				tempPath = new Path(path);
				tempPath.extend(edge);
				if (returnPath.containsKey(edge.target.data)) {
					if (returnPath.get(edge.target.data).compareTo(tempPath) > 0) {
						returnPath.put(edge.target.data, tempPath);
						discovered.add(tempPath);
					}
				} else {
					returnPath.put(edge.target.data, tempPath);
					discovered.add(tempPath);
		...
		// throwing exception if path does not exist
		if (returnPath.get(end) == null) {
			throw new NoSuchElementException();
		}
		return returnPath.get(end);
	}
